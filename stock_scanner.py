...
# --- אבטחה: סיסמה + מפתחות API (נטען בצורה מאובטחת מ-GitHub Secrets) ----
FROM_EMAIL = os.getenv("FROM_EMAIL", "sendemail920@gmail.com")
APP_PASSWORD = os.getenv("EMAIL_PASSWORD")

if not APP_PASSWORD:
    print("⚠️ Warning: EMAIL_PASSWORD environment variable is missing.")
...
# נתיב לוג גנרי שמתאים לריצה בענן
LOGFILE = os.getenv("LOGFILE", "stock_scanner_unified_log.txt")

# טעינת מפתחות ה-API של TwelveData מתוך ה-Secret (מופרדים בפסיק)
API_KEYS = [k.strip() for k in os.getenv("TWELVEDATA_API_KEYS", "").split(",") if k.strip()]
if not API_KEYS:
    print("⚠️ Warning: TWELVEDATA_API_KEYS environment variable is missing or empty.")
